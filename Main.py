...
class AssetManager:
                ...
                if path.is_file():
                    ...
                    if "neutral" in self._characters_expressions[char_id]:
                        self._characters_expressions[char_id][exp_name] = (
                            self._characters_expressions[char_id]["neutral"]
                        )
                    else:
                        self._characters_expressions[char_id][exp_name] = (
                            pygame.Surface((200, 200), pygame.SRCALPHA)
                        )
    ...
    def _init_fonts(self):
        ...
        try:
            ...
            self._fonts["timer"] = pygame.font.SysFont(
                font_head, 48, bold=True
            )
            self._fonts["winner"] = pygame.font.SysFont(font_head, 42)
            self._fonts["sala_title"] = pygame.font.SysFont(font_head, 34)
            self._fonts["medium"] = pygame.font.SysFont(
                font_body, 20, bold=True
            )
            self._fonts["small"] = pygame.font.SysFont(
    ...
    def get_expressions(self, char_id):
        return self._characters_expressions.get(char_id)
...
class StartScene(Scene):
    ...
    def __init__(self, game):
        ...
        self.btn_start = pygame.Rect(
            (game.width // 2) - 125 + 5, 480, 250, 100
        )
    ...
    def draw(self, screen):
        ...
        if img_title:
            ...
            screen.blit(
                scaled_title,
                scaled_title.get_rect(center=(self.game.width // 2, 250)),
            )

        img_portada = self.game.assets.get_img("personajes_portada")
        if img_portada:
            scaled_portada = pygame.transform.smoothscale(
                img_portada, (550, 700)
            )
            screen.blit(
                scaled_portada,
                scaled_portada.get_rect(center=(self.game.width // 2, 400)),
            )
        ...
        self.draw_button(
            screen,
            self.btn_start,
            self.game.assets.get_img("btn_start"),
            (0, 200, 100),
            "INICIAR JUEGO",
            mouse_click and self.btn_start.collidepoint(mouse_pos),
        )
        self.draw_button(
            screen,
            self.btn_exit,
            self.game.assets.get_img("btn_exit"),
            (200, 50, 50),
            "SALIR DEL JUEGO",
            mouse_click and self.btn_exit.collidepoint(mouse_pos),
        )
...
class MenuScene(Scene):
    ...
    def draw(self, screen):
        ...
        title = font_title.render(
            "SELECCIÓN DE PERSONAJES", True, (0, 255, 255)
        )
        screen.blit(
            title, (self.game.width // 2 - title.get_width() // 2, 25)
        )
        ...
        screen.blit(
            turn_text, (self.game.width // 2 - turn_text.get_width() // 2, 80)
        )
        ...
        for i, c in enumerate(self.game.assets.all_characters):
            ...
            is_chosen = (
                self.game.p1.choice and self.game.p1.choice["id"] == c["id"]
            ) or (self.game.p2.choice and self.game.p2.choice["id"] == c["id"])
            ...
            screen.blit(
                num_surf, (cx + card_w // 2 - num_surf.get_width() // 2, cy + 15)
            )

            menu_img = self.game.assets.get_expressions(c["id"])["neutral"]
            screen.blit(
                menu_img,
                menu_img.get_rect(center=(cx + card_w // 2, cy + 200)),
            )

            name_surf = font_title.render(c["name"], True, (255, 255, 255))
            screen.blit(
                name_surf,
                (cx + card_w // 2 - name_surf.get_width() // 2, cy + 370),
            )

            if is_chosen:
                status = self.game.assets.get_font("medium").render(
                    "INICIO", True, (0, 255, 128)
                )
                screen.blit(
                    status,
                    (cx + card_w // 2 - status.get_width() // 2, cy + 425),
                )
...
class CountdownScene(Scene):
    ...
    def draw(self, screen):
        ...
        screen.blit(
            title, (self.game.width // 2 - title.get_width() // 2, 70)
        )

        for p_data, cx in [(self.game.p1, 250), (self.game.p2, 1030)]:
            if p_data.choice:
                img = self.game.assets.get_expressions(p_data.choice["id"])[
                    "euforico"
                ]
                screen.blit(img, img.get_rect(center=(cx, 360)))
                color = (0, 255, 255) if cx == 250 else (255, 0, 128)
                name_surf = self.game.assets.get_font("title").render(
                    f"J: {p_data.name}", True, color
                )
                screen.blit(name_surf, (cx - name_surf.get_width() // 2, 480))

        vs_surf = self.game.assets.get_font("giant").render(
            "VS", True, (255, 255, 255)
        )
        screen.blit(
            vs_surf, (self.game.width // 2 - vs_surf.get_width() // 2, 310)
        )

        count_str = "3" if self.timer > 120 else "2" if self.timer > 60 else "1"
        count_surf = self.game.assets.get_font("giant").render(
            count_str, True, (0, 255, 128)
        )
        screen.blit(
            count_surf,
            (
                self.game.width // 2 - count_surf.get_width() // 2,
                self.game.height // 2 + 180 - count_surf.get_height() // 2,
            ),
        )
...
class PlayScene(Scene):
    ...
    def end_game(self, winner):
        if winner is None:
            self.game.winner_text = (
                "¡EMPATE SENSACIONAL! ¡MISMA CANTIDAD DE CLICS!"
            )
            self.game.winning_character = None
        else:
            self.game.winner_text = (
                f"¡{winner.name} ES EL GANADOR - {winner.score} CLICS EN 10s!"
            )
    ...
    def _draw_cable(
        self, screen, start_p, control_p, end_p, color_core, width=8
    ):
        points = []
        for i in range(26):
            t = i / 25
    ...
    def _get_expression(self, player):
        ...
        score_diff = (
            self.game.p1.score - self.game.p2.score
            if player == self.game.p1
            else self.game.p2.score - self.game.p1.score
        )
        if score_diff > 5:
    ...
    def draw(self, screen):
        ...
        title = self.game.assets.get_font("sala_title").render(
            "SALA DE HACKERS - ¡MÁS CLICS EN 10 SEGUNDOS!", True, (255, 255, 0)
        )
        screen.blit(
            title, (self.game.width // 2 - title.get_width() // 2, 10)
        )
        ...
        screen.blit(
            timer_text,
            (
                timer_box.centerx - timer_text.get_width() // 2,
        ...
        for idx, line in enumerate(
            ["1 class Pixel {", " void run()", " sys.init()", "}"]
        ):
            code_surf = font_small.render(line, True, (30, 65, 75))
            screen.blit(
                code_surf, (term_rect.x + 12, term_rect.y + 15 + idx * 18)
            )
            screen.blit(
                code_surf,
                (term_rect.right - 120, term_rect.y + 15 + idx * 18),
            )

        green = (0, 255, 128)
        screen.blit(
            font_med.render("[ESTADO DEL SISTEMA]:", True, green),
            (term_rect.x + 20, term_rect.y + 15),
        )
        screen.blit(
            font_med.render(
                "MODO: MÁS CLICS EN 10 SEGUNDOS", True, green
            ),
            (term_rect.x + 20, term_rect.y + 40),
        )

        pct = max(
            0.0, min(100.0, (self.remaining_time / self.duration) * 100)
        )
        pct_box = pygame.Rect(
            term_rect.x + 20, term_rect.y + 68, term_rect.width - 40, 26
        )
        ...
        if fill_w > 0:
            ...
            pygame.draw.rect(
                ...
                bar_color,
                pygame.Rect(
                    pct_box.x + 3, pct_box.y + 3, fill_w, pct_box.height - 6
                ),
                border_radius=4,
            )
        ...
        pct_surf = font_small.render(
            f"TIEMPO RESTANTE: {self.remaining_time:.1f}s",
            True,
            (255, 255, 255),
        )
        screen.blit(
            pct_surf,
            (
                pct_box.centerx - pct_surf.get_width() // 2,
                pct_box.centery - pct_surf.get_height() // 2,
            ),
        )
        ...
        btn_img = (
            self.game.assets.get_img("btn_down")
            if self.button_pressed_side
            else self.game.assets.get_img("btn_up")
        )
        if btn_img:
            scaled_btn = pygame.transform.smoothscale(btn_img, (240, 120))
            screen.blit(
                scaled_btn, scaled_btn.get_rect(center=(btn_c_x, btn_c_y))
            )
        else:
            by = 6 if self.button_pressed_side else 0
            brect = pygame.Rect(
                self.game.width // 2 - 80, 455 + by, 160, 60
            )
            pygame.draw.rect(
                screen,
                (255, 128, 0) if self.button_pressed_side else (255, 0, 64),
                brect,
                border_radius=30,
            )
        ...
        screen.blit(
            font_med.render(
                f"J1 - {self.game.p1.name} [A]: {self.game.p1.score} clics",
                True,
                (0, 255, 255),
            ),
            (120, 75),
        )
        txt2 = font_med.render(
            f"J2 - {self.game.p2.name} [L]: {self.game.p2.score} clics",
            True,
            (255, 0, 128),
        )
        screen.blit(txt2, (self.game.width - txt2.get_width() - 120, 75))

        inst = font_med.render(
            "Presiona repetidamente las teclas 'A' (J1) y 'L' (J2)",
            True,
            (255, 255, 255),
        )
        screen.blit(
            inst, (self.game.width // 2 - inst.get_width() // 2, 670)
        )
...
class VictoryScene(Scene):
    ...
    def __init__(self, game):
            ...
            self.confetti.append(
                {
                    ...
                    "y": random.randint(-game.height, 0),
        ...
        self.btn_restart = pygame.Rect(
            (game.width // 2) - 125 + 1, 480, 250, 100
        )
        self.btn_exit = pygame.Rect((game.width // 2) - 125 + 5, 550, 250, 100)
    ...
    def draw(self, screen):
        ...
        for c in self.confetti:
            pygame.draw.rect(
                screen, c["color"], (c["x"], c["y"], c["size"], c["size"])
            )

        win_surf = self.game.assets.get_font("winner").render(
            self.game.winner_text, True, (255, 255, 0)
        )
        screen.blit(
            win_surf, (self.game.width // 2 - win_surf.get_width() // 2, 50)
        )

        for p_data, cx in [(self.game.p1, 380), (self.game.p2, 900)]:
            is_win = (
                self.game.winning_character
                and self.game.winning_character["id"] == p_data.choice["id"]
            )
            ...
            bounce = (
                int(15 * abs(math.sin(pygame.time.get_ticks() * 0.01)))
                if (is_win or is_tie)
                else 0
            )
            ...
            if is_tie:
                ...
                color = (
                    (0, 255, 255)
                    if is_win and cx == 380
                    else (255, 0, 128) if is_win else (200, 100, 100)
                )
        ...
        self.draw_button(
            screen,
            self.btn_restart,
            self.game.assets.get_img("btn_inicio"),
            (0, 200, 100),
            "INICIO",
            m_clk and self.btn_restart.collidepoint(m_pos),
        )
        self.draw_button(
            screen,
            self.btn_exit,
            ...
            (200, 50, 50),
            "SALIR DEL JUEGO",
            m_clk and self.btn_exit.collidepoint(m_pos),
        )
